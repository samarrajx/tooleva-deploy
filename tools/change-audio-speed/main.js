...
ui.btnDownload.addEventListener('click', () => {
    if (processedBlob && currentFile) {
        ...
        downloadBlob(processedBlob, `${baseName}-${targetSpeed}x.mp3`);
    }
});
...
function handleFileSelection(files) {
    ...
    if (!file.type.startsWith('audio/')) {
        showToast('Please upload a valid audio.', 'error');
        return;
    }
...
async function processAudio() {
    ...
    try {
        ...
        showToast('Audio speed changed successfully!');
        
    } catch (err) {
        console.error("FFmpeg Speed Error:", err);
        showToast('Error changing speed. Audio stream missing or format unsupported.', 'error');
        resetUI();
    }
}
