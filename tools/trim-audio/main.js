...
ui.btnDownload.addEventListener('click', () => {
    if (trimmedBlob && currentFile) {
        ...
        downloadBlob(trimmedBlob, `${baseName}-trimmed${fileExt}`);
    }
});
...
async function trimAudio() {
    ...
    try {
        ...
        const cmd = [
            ...
            '-c', 'copy', // Stream copy for speed and lossless trim
            outputName
        ];
        ...
        showToast('Error trimming audio. Stream copy failed. Try another format.', 'error');
        resetUI();
    }
}
