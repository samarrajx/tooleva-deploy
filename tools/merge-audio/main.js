...
ui.btnProcess.addEventListener('click', mergeAudio);

ui.btnDownload.addEventListener('click', () => {
    if (mergedBlob) {
        downloadBlob(mergedBlob, `merged-audio.mp3`);
    }
});
...
async function mergeAudio() {
    ...
    try {
        ...
        mergedBlob = new Blob([fileData.buffer], { type: 'audio/mpeg' });
        
        ui.audioPreview.src = URL.createObjectURL(mergedBlob);
        
        ui.loader.classList.add('hidden');
        ui.loader.classList.remove('flex');
        ui.resultArea.classList.remove('hidden');
        showToast('Audio files merged successfully!');
        
    } catch (err) {
        console.error("FFmpeg Audio Merge Error:", err);
        showToast('Error merging audio files. Formats might be incompatible.', 'error');
        resetUI();
    }
}
